...
These log files are forwarded to Loki by the Grafana Alloy agent running on the host, centralizing them with other system and application logs. The following configuration was added to scrape the Mailman log files:
...
\begin{lstlisting}[caption={Grafana Alloy configuration to forward Mailman logs to Loki.}]
  ...
  loki.write "local" {
    endpoint {
      url = "http://127.0.0.1:8310/loki/api/v1/push"
    }
  }
\end{lstlisting}
